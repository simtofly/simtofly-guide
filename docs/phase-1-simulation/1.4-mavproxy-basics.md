...
**Expected windows:**
1. **Terminal** — SITL output and MAVProxy prompt
2. **Map window** — Shows drone position on OpenStreetMap
3. **Console window** — Shows telemetry data (altitude, speed, etc.)

✅ **Success:** All three windows open, see `STABILIZE>` prompt
...
| **GUIDED** | Computer control via commands | Autonomous commands (takeoff, goto) |
...
- `GUIDED` — For commanded takeoff/landing/movement
- `STABILIZE` — Default starting mode
...
The drone uses:
- **NED frame** — North, East, Down (relative to start position)
- Start position = (0, 0, 0)
- North = positive X
- East = positive Y
- Down = positive Z (altitude is negative Z)
...
**Command format:**
```bash
guided NORTH EAST DOWN
```

**Example — Move 20m North, 10m East, maintain 10m altitude:**
```bash
guided 20 10 -10
```

**What happens:**
- Drone flies to (20N, 10E, 10m altitude)
- Map shows movement
- Takes 10-20 seconds

✅ **Success:** Drone moves to new position
...
**Each command:** Drone flies to that position and holds.
...
**Note:** Always use negative Z for altitude (NED convention).
...
# 5. Move North 20 meters
guided 20 0 -10

# Wait for movement to complete (watch map)

# 6. Move East 20 meters
guided 20 20 -10
...
```bash
echo "custom=YOUR_LAT,YOUR_LONG,ALT,HEADING" >> ~/simtofly_ws/ardupilot/ardupilot/Tools/autotest/locations.txt
```

**Example (replace with your coordinates):**
```bash
echo "myfield=37.7749,-122.4194,50,0" >> ~/simtofly_ws/ardupilot/ardupilot/Tools/autotest/locations.txt
```
...
- [ ] ✅ Can change flight modes (GUIDED, STABILIZE)
...
- ✅ Navigated drone to different positions
- ✅ Landed safely
...
- Install Gazebo Garden 3D simulator
...
**A:** 
- **GUIDED** — Accepts autonomous commands (takeoff, goto, land)
...
### Q: Why does the drone drift after reaching position?

**A:** In SITL with no wind, it shouldn't drift. If it does:
- Check you're in GUIDED or LOITER mode
- STABILIZE mode won't hold position
...
### Drone won't move with guided command
...
mode GUIDED
```
Then send guided command again.
